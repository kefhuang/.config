...
3. Resolve `scripts/run_consult.py` relative to this skill directory, then run it outside the sandbox with escalated permissions. Pass the shared brief with exactly one of `--context-file` or `--context-stdin`. Always redirect stdout to a **unique** temporary file generated via `mktemp` (e.g. `OUT=$(mktemp -t consult_result.XXXXXX.json)` then `... > "$OUT"`) and read it back afterward — direct stdout capture may not work reliably. Never use a fixed path like `/tmp/consult_result.json`, since concurrent consults would overwrite each other.
